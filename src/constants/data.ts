    ...
    details: [
      {
        ...
        problems: [
          '프론트엔드에서 API 로직을 작성하려면 백엔드 개발 이후에 작성해야하기에, 이로 인해 개발 주기가 비효율적으로 흘러 가는 불편함이 있었습니다.',
        ],
        solves: [
          'MSW를 사용하여 Mock API를 만듦으로써 서버 의존성 없이 API 로직을 작성할 수 있었고, 추후 End Point만 교체하는 것으로 실제 서버와의 통신이 가능하였습니다.',
        ],
      },
      {
      ...
      },
      {
        ...
        problems: [
          '다른 분야 직군과 진행 상황을 공유할 때 서로의 전문성이 달라 의사소통을 하는데 어려움이 있을 것 같았습니다.',
        ],
        solves: [
          'Storybook을 통해 컴포넌트를 시각적으로 검토하고 테스트할 수 있는 환경을 제공함으로써 보다 원할한 의사소통을 가능하게 하였습니다.',
        ],
      },
      {
        title: '4. 반응형 웹 적용',
    ...
    details: [
      {
        ...
        problems: [
          '카드 등록 폼의 특성상 입력 형식이 까다롭고 복잡하기에, 잘못 입력 했을 경우 사용자에게 즉각적인 피드백이 필요했습니다.',
        ],
        solves: [
          'React의 제어 컴포넌트 방식을 활용하여 사용자의 입력이 발생할 때마다 유효성 검사를 실시하고, 입력값이 잘못 되었을 경우 즉각적인 피드백을 통해 빠른 수정을 유도할 수 있었습니다.',
        ],
      },
      {
        title: '2. Auto Focus, Numeric Input을 통한 모바일 사용성 증진',
        problems: [
          '모바일 환경에서 카드 정보를 입력할 때 일일히 입력 창을 클릭해야하는 불편함이 있었습니다.',
          '모바일 환경은 가상 키보드를 사용하기에 각 폼에서 요구하는 정보에 맞게 숫자 키보드, 문자 키보드 등으로 변환할 필요성이 있었습니다.',
        ],
        solves: [
          'React의 Ref와 제어 컴포넌트 방식을 활용하여 Auto Focus를 구현함으로써, 모바일 환경에서 입력 창을 클릭하는 과정 없이 보다 편리하게 입력할 수 있도록 하였습니다.',
          'input 태그의 type 속성을 활용하여 각 입력 폼에 맞게 가상 키보드의 종류를 변경함으로써, 보다 편리하게 입력할 수 있도록 하였습니다.',
        ],
      },
      {
        ...
        solves: [
          'Storybook을 사용한 CDD를 통해 컴포넌트를 시각적으로 문서화하고 조작해볼 수 있는 환경을 제공함으로써 보다 명확한 의사소통을 가능하게 하였습니다.',
        ],
      },
      {
        ...
        solves: [
          'Modal 컴포넌트의 Interface를 조정하고 React의 Portal를 활용하여 타 컴포넌트와의 의존성을 제거함으로써, 미션 전반에서 공용적으로 사용할 수 있도록 하였습니다.',
        ],
        result: {
          description: {
            name: 'NPM',
    ...
    details: [
      {
        ...
        solves: [
          'IntersectionObserver를 활용하여 사용자가 마지막 영화 목록까지 스크롤 하였을 때 자동으로 추가 조회를 수행할 수 있도록 하였습니다.',
        ],
      },
      {
        ...
        solves: [
          'Skeleton UI를 도입하여 영화 포스터 이미지가 로딩 중임을 사용자가 보다 명확하게 인지할 수 있도록 하고, Layout Shift를 개선하여 CLS 0을 달성하였습니다.',
        ],
        result: {
