    ...
    details: [
      {
        ...
        solves: [
          '지도의 줌 레벨에 따라 마커의 지름이 차지하는 실제 거리를 구하고, 마커 간 겹침 여부를 판단하여 마커 클러스터링을 구현했습니다.',
      ...
      },
      {
        ...
        solves: [
          'CSS의 Media Query, Web API의 MatchMedia를 사용하여 PC, 태블릿, 모바일 각각의 환경에 적합한 사용성을 제공할 수 있도록 했습니다.',
        ],
        result: {
    ...
    details: [
      {
        ...
        problems: [
          '카드 등록 폼의 특성상 입력 형식이 까다롭고 복잡하기에, 사용자가 잘못된 형식으로 정보를 입력하는 경우가 발생할 가능성이 높았습니다. 제출 버튼을 누룬 후에 잘못된 입력값을 표시하는 방법은 사용자가 잘못 입력한 값을 찾아 다시 한번 수정해야 하므로 번거로워 보였습니다. 따라서 사용자가 잘못된 값을 입력했을 경우 그 즉시 사용자에게 피드백을 주어 수정을 유도하면 사용자 경험이 향상될 것이라 생각했습니다.',
        ],
        solves: [
